...
<div class="row">
    <div class="col-md-8">
        <div class="card p-4">
            <h4 class="fw-bold mb-4">Edit Meal</h4>
            ...
            <form action="{{ route('meals.update', $meal) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description" rows="3" required>{{ old('description', $meal->description) }}</textarea>
                    @error('description')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    @if($meal->image)
                        <img src="{{ asset('storage/' . $meal->image) }}" alt="{{ $meal->name }}" class="img-thumbnail mb-2" style="max-width: 200px;">
                    @else
                ...
                <div class="d-flex gap-2">
                    <button type="submit" class="btn btn-primary px-4">Update Meal</button>
                    <a href="{{ route('meals.index') }}" class="btn btn-outline-secondary">Cancel</a>
                </div>
            </form>
        </div>
    </div>
